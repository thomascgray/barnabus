...
- Admin/board HTTP API (`/api/admin/boards`, `/api/boards/:id`, `/api/boards/:id/images`) is gated by `requireAdmin` (`BARNABUS_ADMIN_SECRET`) where applicable; `docs/self-hosting-plan.md` tracks the phase plan (Phases 0–4 done, packaging is Phase 5).
...
- Self-hosting effort: multi-board rooms, passphrases, admin API, presence, and image upload are **done** (Phases 0–4); packaging/docs is Phase 5. See `docs/self-hosting-plan.md` for the plan, and `./CLAUDE/frontend.md` / `./CLAUDE/backend.md` for the principles.
